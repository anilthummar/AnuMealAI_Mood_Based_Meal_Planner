...
class HomeCubit extends Cubit<HomeState> {
  ...
  Future<void> loadDashboard() async {
    ...
    try {
      ...
      final mealsCount = await profileRepository.getMealsCookedCount();
      final ingredients = await ingredientRepository.getIngredients();
      final availableIngredients = ingredients.where((i) => i.isAvailable).toList();
      final availableNames = availableIngredients.map((i) => i.name).toList();

      final currentMood = moodRepository.getSelectedMood() ?? moodRepository.getAllMoods().first;
      ...
      final preferredCuisine = favCuisines.isNotEmpty ? favCuisines.first : 'Italian';
